...
using OnDataReceiveCallback = std::function<void(const char *data, int dataLen, const SocketAddress &fromAddress)>;
...
class Host
{
  public:
    ...
    int SendData(const char *data, int dataLen, const SocketAddress &toAddress) const;
  ...
  private:
    ...
    void OnDataReceived(const SocketAddress &fromAddress, const char *data, int readByteCount) const;
