...
TEST_CASE("HostClientSendDataToServerTest::UDP", "[.udp]")
{
    ...
    auto onServerDataReceivedCallback = [&](const char *data, int dataLen, const Chatik::SocketAddress &fromAddress) {
    ...
    const Chatik::SocketAddress serverAddress(Chatik::SocketUtil::CreateIPv4FromString(kServerAddress));

    const int sentDataLen = hostClient.SendData(expectedData, sizeof(expectedData), serverAddress);
...
TEST_CASE("HostServerSendDataBackToClientTest::UDP", "[.udp]")
{
    ...
    auto onClientDataReceivedCallback = [&](const char *data, int dataLen, const Chatik::SocketAddress &fromAddress) {
        assert(data && dataLen > 0);
        memcpy(actualData, data, dataLen);
    ...
    int sentDataLen = hostClient.SendData(clientData, sizeof(clientData), serverAddress);
    ...
    const Chatik::SocketAddress clientAddress(Chatik::SocketUtil::CreateIPv4FromString(kClientAddress));

    sentDataLen = hostServer.SendData(serverData, sizeof(serverData), clientAddress);
...
TEST_CASE("HostServertSendDataToClientTest::TCP", "[.tcp]")
{
    ...
    const int sentDataLen = hostServer.SendData(expectedData, sizeof(expectedData), clientAddress);
